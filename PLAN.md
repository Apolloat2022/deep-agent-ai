...
A two tier split rather than a single model:

1. Orchestrator: Claude Opus 4.8. Planning, delegation, and approval sensitive tool calls benefit most from the strongest reasoning, and the orchestrator emits comparatively few tokens.
2. Subagents: Claude Sonnet 5. Subagents do the bulk of token consumption (research, drafting, extraction). Sonnet 5 delivers near Opus quality on agentic work at meaningfully lower cost than Opus.

**Serving path: Amazon Bedrock, not the first party Anthropic API.** This was a deliberate choice made once real AWS access was available (Session 4): the existing AWS account already runs another service (`riskguard-ai`) that authenticates to Claude via Bedrock using its ECS task role's IAM permissions, with no API key to provision, rotate, or store as a secret. Matching that pattern means one fewer credential to manage and consistency with how this account already runs Claude workloads. Both target models are available in this account only as `us-east-1` cross region inference profiles (`us.anthropic.claude-opus-4-8`, `us.anthropic.claude-sonnet-5`), not bare on demand model IDs — confirmed via `aws bedrock get-foundation-model`, not assumed. Model access for both was a separate entitlement from IAM and required explicitly accepting Anthropic's Bedrock usage agreement (standard published pricing, not a special rate) — see `HANDOFF.md` for the exact commands and current propagation status.
...
5. Done (Sonnet, after a stalled Haiku session): Dockerfile and `.dockerignore`, ECS task definition with placeholder ARNs/URIs for the operator to fill in, `/healthz` endpoint (verified 200 in-process), a CI workflow, and a ruff formatting pass — all confirmed not to break the existing test suite (13 passed, 3 skipped throughout).
6. Done (Sonnet, with real AWS access): migrated model serving from the first party Anthropic API to Amazon Bedrock at the operator's direction, matching this AWS account's existing pattern; discovered real account resources via read only AWS calls before changing anything; created a dedicated ECR repository, CloudWatch log group, and two least privilege IAM roles (mirroring the account's existing `riskguard-task-role` / `riskguard-execution-role` scoping pattern exactly); accepted the Bedrock model access agreements for both target models; filled in `deploy/task-definition.json` and the CI workflow with real ARNs and values everywhere possible. Full account discovery, every value filled in versus left as an explicit placeholder and why, and current Bedrock entitlement propagation status are in `HANDOFF.md`.
7. Done (Sonnet): operator provisioned a dedicated Neon Postgres instance; the `DATABASE_URL` secret was created and `deploy/task-definition.json` registered with ECS for real (revision 1, `ACTIVE`) — no placeholders left anywhere in that file. `python-dotenv` wired into `agent.py`, `service/persistence.py`, and `service/clients.py` so a project root `.env` loads automatically in local development. This also produced the project's first genuine, successful end to end verification of the `AGENT_ENV=prod` persistence path against a real Postgres instance (Docker had been unavailable all project). Full detail, including a Windows-only event loop caveat surfaced during that test, in `HANDOFF.md` Session 5.
8. Done (Sonnet): operator confirmed the Opus 4.8 / Sonnet 5 Bedrock access block independently (identical failure across two API surfaces, hours after `AUTHORIZED` control-plane status) and approved a temporary switch to Sonnet 4.5, which is applied now with revert instructions inline in `agent.py`. Testing that switch was the first time this project ran `python agent.py` against a real, working model, which surfaced and fixed a genuine bug: `build_agent()`'s persistence fallback constructed a synchronous `SqliteSaver`, incompatible with this graph's async-only tools, silently broken since Session 2 (when the tools became async-only) but never exercised until now. `build_agent()` now requires the caller to supply async persistence explicitly (no default), matching the pattern `service/app.py` already used correctly. Result: `python agent.py` completes a full real run for the first time in this project's history, and `pytest tests/` passes all 16 tests with 0 skipped, including the three HITL integration tests that had never had a working model to run against. The Opus 4.8 / Sonnet 5 access issue itself is unresolved and needs a support case — see `HANDOFF.md` Session 4's evidence table and Session 6 for the full account.
